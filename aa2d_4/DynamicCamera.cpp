...
{
	DynamicCamera(Vector_(0,0,0));
}

DynamicCamera::DynamicCamera(Vector_ &vecSrc)
{
	camSrc = vecSrc;
	camAng.Set(0, 0, 0);
	viewMat.Identity();
...
void DynamicCamera::Update()
{
	...
	Matrix_	angRotMat(camAng.x, camAng.y, camAng.z);
	...
	Vector_	sideVec(viewMat.p[0], viewMat.p[1], viewMat.p[2]);
	Vector_	upVec(viewMat.p[3], viewMat.p[4], viewMat.p[5]);
	Vector_	dirVec(viewMat.p[6], viewMat.p[7], viewMat.p[8]);
