...
glmFirstPass(GLMmodel* model, FILE* file) 
{
  ...
  /*if (!textured)
  {
	  ...
	  m_baiscobj->LoadT16("texture.bmp",tex_antt);
	  textured = true;
  }  */  
}
...
glmReadOBJ(char* filename)
{
	...
	/*m_baiscobj2 = new baiscobj();
	m_baiscobj2->LoadT16(name,tex_antt[i]);
	index = i;*/
 ...
 */
GLvoid
glmDraw(GLMmodel* model, GLuint mode, GLuint tex)
{
    static GLuint i;
 ...
 */
GLuint
glmList(GLMmodel* model, GLuint mode, GLuint tex)
{
    GLuint list;
    
    list = glGenLists(1);
    glNewList(list, GL_COMPILE);
    glmDraw(model, mode, tex);
    glEndList();
